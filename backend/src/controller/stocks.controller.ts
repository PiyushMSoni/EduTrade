...
const addOrUpdatePosition = async (user: any, symbol: string, price: number, quantity: number) => {
    const existingPosition = user.positions.find((pos: IPosition) => pos.symbol === symbol);

    if (existingPosition) {
        // Update existing position
        existingPosition.quantity += quantity;
        existingPosition.purchasePrice = price; // You might want to average this price if needed
    } else {
        // Create new position
	...
	*/
	...
	try {
		...
		if (user.cash! < price * quantity) {
			...
			// Use the new addOrUpdatePosition function
			addOrUpdatePosition(user, symbol, price, quantity);
