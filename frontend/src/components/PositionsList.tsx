...
function PositionsList() {
	...
	useEffect(() => {
		const fetchPortfolio = async () => {
			try {
				const { positions } = await accounts.getPortfolio(); // Ensure your service returns updated positions
				...
				{isLoading ? (
					...
					<Stack divider={<StackDivider />} spacing="4">
						{positions.map((position) => {
							const gainLoss =
								(position.regularMarketPrice - position.purchasePrice) * position.quantity;
